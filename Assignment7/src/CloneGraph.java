...
//node. 
// 
// Related Topics 深度优先搜索 广度优先搜索 图 哈希表 👍 484 👎 0
...
class Solution {
    ...
    public Node cloneGraph2(Node node) {
        ...
        while (queue.size() > 0) {
            ...
            if (!nodeMap.containsKey(temp.val)) {
                nodeMap.put(temp.val, new Node(temp.val));
            }
            Node newNode = nodeMap.get(temp.val);
            for (Node neighbour: temp.neighbors) {
                ...
                newNode.neighbors.add(nodeMap.get(neighbour.val));
            }
        }
